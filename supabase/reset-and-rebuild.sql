-- ============================================================================
--  【注意】このファイルは、プロジェクトの中身を一度すべて空にしてから
--  最新の設計で作り直す「リセット専用」のスクリプトです。
--
--  実行すると、このプロジェクトに入っているデータ（スタッフ・顧客・予約など）は
--  すべて削除されます。データが消えても問題ないプロジェクトでのみ実行してください。
--  （通常の schema.sql は「まだ何もないプロジェクト」専用で、既にテーブルが
--  存在していると新しい列が追加されずエラーになることがあるため、
--  このファイルは先に古いテーブルを削除してから作り直します）
-- ============================================================================

drop view if exists public_availability cascade;
drop view if exists public_staff cascade;
drop function if exists public_find_customer_by_phone(text) cascade;
...
-- ============================================================================
--  ここから下は schema.sql と同じ内容です（最新の設計を最初から作成します）
-- ============================================================================
...
-- ---------------------------------------------------------------------------
-- HotPepper Beauty（SALON BOARD）メール連携の処理ログ
-- ---------------------------------------------------------------------------
